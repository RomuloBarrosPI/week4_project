...
# Get a list of all the files containing the train sets in order to use them as the guide to find their test sets peers
...
# Use the edited variable names as labels to the factors created for the activity column of merged_data
...
final_data <- aggregate(merged_data[,3:68], list(merged_data$subject, merged_data$activity), "mean") %>% rename(subject = Group.1, activity = Group.2) %>% arrange(subject)
